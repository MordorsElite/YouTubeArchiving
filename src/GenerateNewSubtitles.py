...
def extractAudioFile(video_file:str, output_audio_file:str) -> None:
    # Load the video file
...
def getWordByWordTimestamps(model, audio_file:str):
...
def generateVTT(word_by_word_timestamps, output_subtitle_file:str):
...
def generateNewSubtitles(video_file:str, output_subtitle_file:str) -> None:
    # Extract audio file
    temp_audio_file = os.path.join('TempFiles', f'{video_file}.m4a')
    extractAudioFile(temp_audio_file)

    # Generate Transcription
    model = load_model()
    result = getWordByWordTimestamps(model, temp_audio_file)

    # Generate Subtitle file from Transcriptions
    generateVTT(result, output_subtitle_file)

    # Delete temporary audio file
    deleteFile(temp_audio_file)
...
if __name__ == '__main__':
    ...
    result = getWordByWordTimestamps(model, audio_file)
